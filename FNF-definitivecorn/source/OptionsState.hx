...
{
	var options:Array<String> = ['Notes', 'Controls', 'Preferences', 'Change Language'];
	private var grpOptions:FlxTypedGroup<Alphabet>;
	private static var curSelected:Int = 0;
	public static var menuBG:FlxSprite;
	var menuBg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('backgrounds/Spike'));
	...
	override function create() {
		...
		menuBG = new FlxSprite().loadGraphic(Paths.image('backgrounds/Spike'));
		...
		{
			var optionText:Alphabet = new Alphabet(0, 0, LanguageManager.getTextString('option_' + options[i]), true, false);
...
class NotesSubstate extends MusicBeatSubstate
{
	...
	public function new() {
		...
		hsvText = new Alphabet(0, 0, LanguageManager.getTextString('option_hsvText'), false, false, 0, 0.65);
		add(hsvText);
		changeSelection();
	}
...
class ControlsSubstate extends MusicBeatSubstate {
	...
	public function new() {
		...
		for (i in 0...optionShit.length) {
			...
			var optionText:Alphabet = new Alphabet(0, (10 * i), LanguageManager.getTextString('option_' + optionShit[i]), (!isCentered || isDefaultKey), false);
...
class PreferencesSubstate extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		for (i in 0...options.length)
		{
			...
			var optionText:Alphabet = new Alphabet(0, 70 * i, LanguageManager.getTextString('option_' + options[i]), false, false);
			optionText.isMenuItem = true;
			if(isCentered) {
				optionText.screenCenter(X);
				optionText.forceX = optionText.x;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			case 'Framerate':
				daText = LanguageManager.getTextString('option_framerateTxt');
			case 'Note Delay':
				daText = LanguageManager.getTextString('option_notedelayTxt');
			case 'FPS Counter':
				daText = LanguageManager.getTextString('option_fpscounterTxt');
			case 'Low Quality':
				daText = LanguageManager.getTextString('option_lowqualityTxt');
			case 'Persistent Cached Data':
			    daText = LanguageManager.getTextString('option_cachedTxt');
			case 'Anti-Aliasing':
				daText = LanguageManager.getTextString('option_antialiasingTxt');
			case 'Downscroll':
				daText = LanguageManager.getTextString('option_downscrollTxt');
			case 'Middlescroll':
				daText = LanguageManager.getTextString('option_middlescrollTxt');
			case 'Ghost Tapping':
				daText = LanguageManager.getTextString('option_ghostTxt');
			case 'Swearing':
				daText = LanguageManager.getTextString('option_swearingTxt');
			case 'Violence':
				daText = LanguageManager.getTextString('option_violenceTxt');
			case 'Note Splashes':
				daText = LanguageManager.getTextString('option_splashesTxt');
			case 'Flashing Lights':
				daText = LanguageManager.getTextString('option_flashTxt');
			case 'Camera Zooms':
				daText = LanguageManager.getTextString('option_cameraTxt');
			case 'Hide HUD':
				daText = LanguageManager.getTextString('option_hidehudTxt');
			case 'Cutscenes':
				daText = LanguageManager.getTextString('option_cutscenesTxt');
			case 'Hitsound':
                daText = LanguageManager.getTextString('option_hitsoundTxt');
			case 'Hide Song Length':
				daText = LanguageManager.getTextString('option_songlengthTxt');
		}
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Hitsound':
					    daValue = FlxG.save.data.donoteclick;
					case 'Flashing Lights':
