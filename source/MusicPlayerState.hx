...
class MusicPlayerState extends MusicBeatState
{
    ...
    override function create()
    {
        ...
        var secretSongs:Array<Dynamic> = [
            ['rage', 'ragebambi'],
            ['icsa3aahcmaj', 'dave_plays'],
            ['agronomo', 'martielo'],
            ['talkative', 'discordkun'],
            ['untitled', 'carafino'],
        ];
        for (i in 0...secretSongs.length)
        {
            ...
            {
                case 'rage', 'ragebambi': FlxG.save.data.probabilityFound;
                case 'icsa3aahcmaj':  FlxG.save.data.spinFound;
                case 'agronomo': FlxG.save.data.agronomoFound;
                case 'talkative': FlxG.save.data.missassholdFound;
                case 'untitled': FlxG.save.data.fancyFound;
                default: false;
            }
        if (unlockSong)
            {
                var bad = secretSongs[i][2] != null ? secretSongs[i][2] : false;
                songs.push(new PlaySongMetadata(secretSongs[i][0], false, secretSongs[i][1], bad, secretSongs[i][0] != 'in the trap'));
                if (secretSongs[i][0] != 'in the trap')
                {
        ...
        bg = new FlxSprite().loadGraphic(Paths.image('backgrounds/Spike'));
        ...
        iconP1 = new HealthIcon("speaker", true);
		iconP1.y = healthBar.y - (iconP1.height / 2);
		add(iconP1);

		iconP2 = new HealthIcon("speaker", false);
